...
Write-Header "Microsoft Security Stack - Role Assignments Export v2.0  [7 Sections]"
...
if ($IncludeXDRRBAC) {
    ...
    # -------------------------------------------------------------------------
    # 6a - Defender XDR Unified RBAC Custom Roles
    # NOTE: No supported API endpoint exists for querying Unified RBAC custom
    # roles via application-only auth. Export manually from:
    # Defender portal > System > Permissions > Roles > Export
    # -------------------------------------------------------------------------
    Write-Host ""
    Write-Host "  6a - Defender XDR Unified RBAC Custom Roles" -ForegroundColor Cyan
    Write-Host "  NOTE: Custom roles defined in the Defender portal cannot be" -ForegroundColor DarkYellow
    Write-Host "  exported programmatically. Please export manually from:" -ForegroundColor DarkYellow
    Write-Host "  Defender portal > System > Permissions > Roles > Export" -ForegroundColor DarkYellow
    Write-Host ""
    ...
    # -------------------------------------------------------------------------
    # 6b - XDR Identity Audit via Advanced Hunting (KQL)
    # Queries IdentityInfo table for identities with XDR-relevant Entra roles.
    # Cross-references Section 1 to flag identities not captured via direct
    # Graph query (e.g. nested group members) -- these appear as blind spots.
    # -------------------------------------------------------------------------
    Write-Step "6b - XDR Identity Audit via Advanced Hunting (KQL)..."
    ...
    try {
        if (-not $Script:secToken) {
            Write-Warn "  6b - Skipped: no XDR Security token."
...
try {
    ...
    # -------------------------------------------------------------------------
    # 7b -- Azure RBAC Role Assignments for Service Principals (requires Az session)
    # -------------------------------------------------------------------------
    if ($SentinelWorkspaces.Count -gt 0 -or $ScanDefenderForCloud) {
        ...
        try {
            ...
            foreach ($sub in $allAzSubs) {
                ...
                foreach ($ra in $assignments) {
                    ...
                    $isRunningApp = ($sp.AppId -eq $AppClientId)
                    $spResults += [PSCustomObject]@{
                        SPDisplayName    = if ($ra.DisplayName) { $ra.DisplayName } elseif ($sp) { $sp.DisplayName } else { "[Orphaned - SP not found: $($ra.ObjectId)]" }
                        AppId            = if ($sp) { $sp.AppId } elseif ($ra.ObjectId) { "[ObjectId: $($ra.ObjectId)]" } else { "[Unknown]" }
    ...
    # -------------------------------------------------------------------------
    # 7c -- Sensitive API Permissions granted to Service Principals
    # -------------------------------------------------------------------------
